...
/**
 * git's own stderr is CAPTURED, not passed through. Every git failure this script can provoke is one
 * it already handles — a graded commit rebased away throws, is caught, and renders as "re-check" —
 * so `fatal: bad revision` is not news, it is the sound of the check working. Left inherited it put
 * forty "fatal:" lines above the address in the window an owner opens by double-clicking (measured on
 * a 14-row board), which also made the rule serve.mjs needs impossible to state: with the noise gone,
 * ANYTHING this script prints is something it meant to say, and can be shown to the owner as-is.
 */
...
// tmp + rename, like every other store in this family. The page loads this file with a <script> tag,
// so a half-written one does not show an old number, it blanks the board — and two windows writing
// at once stopped being hypothetical the day serve.mjs began running this on every start.
writeFileSync(join(here, 'staleness.js.tmp'), out);
renameSync(join(here, 'staleness.js.tmp'), join(here, 'staleness.js'));
